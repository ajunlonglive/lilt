...
PHP_API zval *FUNC(read_property, zval *object, zval *member, int type, void **cache_slot, zval *rv) {
    ...
    if (zend_string_equals_literal(property_name, "byReference")) {
        ZVAL_BOOL(rv, intern->arg_info->pass_by_reference);
    } else if (zend_string_equals_literal(property_name, "name")) {
        ZVAL_STR(rv, intern->arg_info->name);
    } else if (zend_string_equals_literal(property_name, "nullable")) {
        ZVAL_BOOL(rv, intern->arg_info->allow_null);
    } else if (zend_string_equals_literal(property_name, "typeHint")) {
        ZVAL_BOOL(rv, intern->arg_info->type_hint);
    } else if (zend_string_equals_literal(property_name, "variadic")) {
        ZVAL_BOOL(rv, intern->arg_info->is_variadic);
    }
